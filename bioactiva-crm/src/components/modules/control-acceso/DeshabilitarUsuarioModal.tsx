            ...
            <div className="relative z-10 w-full max-w-sm bg-white rounded-2xl shadow-2xl">
                ...
                <div className="px-6 py-5 space-y-4">
                    <p className="text-sm text-gray-600">
                        {esActivo
                            ? <><span>¿Estás seguro de que deseas deshabilitar a </span><span className="font-semibold text-gray-900">{nombreCompleto}</span><span>? El usuario no podrá iniciar sesión.</span></>
                            : <><span>¿Deseas habilitar nuevamente a </span><span className="font-semibold text-gray-900">{nombreCompleto}</span><span>? El usuario podrá volver a iniciar sesión.</span></>
                        }
                    </p>
                    <div className="flex items-center justify-end gap-3">
                        ...
                        <button type="button" onClick={handleConfirm} disabled={isLoading}
                            className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-xl transition-colors disabled:cursor-not-allowed ${esActivo ? 'bg-red-500 hover:bg-red-600 disabled:bg-red-200' : 'bg-[#1C7E3C] hover:bg-[#16642f] disabled:bg-[#BCF7B3]'}`}>
                            {isLoading ? <><Loader2 size={14} className="animate-spin" />Procesando...</> : (esActivo ? 'Sí, deshabilitar' : 'Sí, habilitar')}
                        </button>
                    </div>
                </div>
            </div>
