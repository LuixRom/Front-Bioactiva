...
export function EditarUsuarioModal({ usuario, isLoading, error, onClose, onSubmit }: Props) {
    const { register, handleSubmit, formState: { errors }, reset } = useForm<EditarUsuarioFormValues>({
            ...
            <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl">
                ...
                <form onSubmit={handleSubmit(handleFormSubmit)} className="px-6 py-5 space-y-4">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input type="text" placeholder="Nombres y apellidos" {...register('nombre_completo')}
                            className={`w-full px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 rounded-xl border-2 outline-none transition-colors bg-gray-50 ${errors.nombre_completo ? 'border-red-400 focus:border-red-500' : 'border-gray-200 focus:border-[#1C7E3C]'}`} />
                        {errors.nombre_completo && <p className="text-red-500 text-xs">{errors.nombre_completo.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide">Correo institucional</label>
                        <input type="email" placeholder="usuario@bioactiva.pe" {...register('correo')}
                            className={`w-full px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 rounded-xl border-2 outline-none transition-colors bg-gray-50 ${errors.correo ? 'border-red-400 focus:border-red-500' : 'border-gray-200 focus:border-[#1C7E3C]'}`} />
                        {errors.correo && <p className="text-red-500 text-xs">{errors.correo.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide">Rol</label>
                        <select {...register('rol')} className={`w-full px-4 py-2.5 text-sm text-gray-900 rounded-xl border-2 outline-none transition-colors bg-gray-50 ${errors.rol ? 'border-red-400 focus:border-red-500' : 'border-gray-200 focus:border-[#1C7E3C]'}`}>
                    ...
                    <div className="flex items-center justify-end gap-3 pt-2">
                        ...
                        <button type="submit" disabled={isLoading} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-[#1C7E3C] hover:bg-[#16642f] disabled:bg-[#BCF7B3] disabled:cursor-not-allowed rounded-xl transition-colors">
                            {isLoading ? <><Loader2 size={14} className="animate-spin" />Guardando...</> : 'Guardar cambios'}
                        </button>
                    </div>
                </form>
            </div>
